...
public class BrandDeletedNotificationHandlerTests
{
    ...
    public async Task Handle_ResolvesRecipients_ExcludesActor_CallsSendNotification()
    {
        ...
        var sut = new BrandDeletedNotificationHandler(_notifManager.Object, _resolver.Object);
        var evt = new BrandDeletedEvent(10, "Puma", actor);

        await sut.HandleAsync(evt);

        _notifManager.Verify(m => m.SendNotification(
            "Marka silindi",
            It.Is<string>(s => s.Contains("Puma")),
    ...
    public async Task Handle_NoRecipientsAfterActorFilter_DoesNothing()
    {
        ...
        var sut = new BrandDeletedNotificationHandler(_notifManager.Object, _resolver.Object);
        await sut.HandleAsync(new BrandDeletedEvent(1, "X", actor));
