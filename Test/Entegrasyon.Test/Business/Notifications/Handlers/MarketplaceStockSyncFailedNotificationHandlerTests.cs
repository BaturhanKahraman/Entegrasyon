...
public class MarketplaceStockSyncFailedNotificationHandlerTests
{
    ...
    public async Task Handle_ResolvesRecipients_CallsSendNotification()
    {
        ...
        var sut = new MarketplaceStockSyncFailedNotificationHandler(_notifManager.Object, _resolver.Object);
        var evt = new MarketplaceStockSyncFailedEvent(1, productId, "Timeout");

        await sut.HandleAsync(evt);

        _notifManager.Verify(m => m.SendNotification(
            "Stok senkronizasyonu başarısız",
    ...
    public async Task Handle_NoRecipients_DoesNothing()
    {
        ...
        var sut = new MarketplaceStockSyncFailedNotificationHandler(_notifManager.Object, _resolver.Object);
        await sut.HandleAsync(new MarketplaceStockSyncFailedEvent(1, Guid.NewGuid(), "err"));
